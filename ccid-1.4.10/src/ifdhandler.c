...
RESPONSECODE VdrDisablePPS(int readerIndex)
{
	unsigned char 	abCmd[8];
	unsigned char 	abBuf[256];
	unsigned int 	lRtnLen;
	RESPONSECODE 	status;
	//unsigned char  	bCardStatus;
	
	abCmd[0] = 0x40;
	abCmd[1] = 0x57;
	abCmd[2] = 0x01;  //Disable PPS
	abCmd[3] = 0x00;
	abCmd[4] = 0x00;
	abCmd[5] = 0x00;
	abCmd[6] = 0x00;
	...
	status = CmdEscape(readerIndex, abCmd, 8, abBuf, &lRtnLen);	
	
	return status;
}

RESPONSECODE VdrEnablePPS(int readerIndex)
{
	unsigned char  	abCmd[8];
	unsigned char  	abBuf[256];
	unsigned int   	lRtnLen;
...
RESPONSECODE VdrSetT1(int readerIndex, BYTE bMinorVer)
{
	unsigned char  	abCmd[8];
	unsigned char  	abBuf[256];
	unsigned int   	lRtnLen;
	...
	abCmd[0] = 0x40;
	abCmd[1] = 0xC6;
	abCmd[2] = 0x05; // 0x04 = write, 0x00 = read
	if(bMinorVer == 0x12)
	{
	    abCmd[3] = 0xb3;
	}
	...
	UCHAR Flags, UCHAR PTS1, UCHAR PTS2, UCHAR PTS3)
{
	...
	unsigned char 	TxBuffer[8] = {0x40,0xc7,0x00,0x00,0x00,0x00,0x00,0x00};
	...
	/* sned escape to get FwInfo.bMinorVer */
	 CmdEscape(reader_index,TxBuffer,8,RxBuffer,&RxLength);
	/*get poweron ATR from global CcidSlots[reader_index].pcATRBuffer*/
	if((RxBuffer[7] > 0x11) && (RxBuffer[7] < 0x16))
			{
				unsigned char CompareATR[23] = {0x3B, 0x9F, 0x95, 0x81, 0x31, 0xFE, 0x9F, 0x00, 0x65, 0x46, 0x53, 
												0x05, 0x30, 0x06, 0x71, 0xDF, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0xB6};
 			        if(!memcmp(CompareATR, CcidSlots[reader_index].pcATRBuffer, 23))
 			        {
 			            VdrEnablePPS(reader_index);
 			        }
			}
	...
	*AtrLength = 0;
	...
	switch (Action)
	{
		...
		case IFD_RESET:
			 ...
			 * before a power on */
			 /*send escape command to get fwinfo ,return buffer byte 7 are the bMinorVer*/
			return_value = CmdEscape(reader_index,TxBuffer,8,RxBuffer,&RxLength);

			/*power on to get ATR*/

			return_value = CmdPowerOn(reader_index, &nlength, pcbuffer,PowerOnVoltage);
			/*modify by michael.zhang 2013.7.2*/
			if((RxBuffer[7] > 0x11) && (RxBuffer[7] < 0x16))
			{
				unsigned char CompareATR[23] = {0x3B, 0x9F, 0x95, 0x81, 0x31, 0xFE, 0x9F, 0x00, 0x65, 0x46, 0x53, 
												0x05, 0x30, 0x06, 0x71, 0xDF, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0xB6};
 			        if(!memcmp(CompareATR, pcbuffer, 23))
 			        {
 			            VdrDisablePPS(reader_index);
						return_value = CmdPowerOn(reader_index, &nlength, pcbuffer,PowerOnVoltage);
 			            return_value = VdrSetT1(reader_index, RxBuffer[7]);						
 			        }
			}
...
/*modify by michael 2013.6.19*/
if (IOCTL_SMARTCARD_POWER == dwControlCode)	
	{		
		unsigned int nlength;
		unsigned char pcbuffer[10+MAX_ATR_SIZE];
		unsigned int oldReadTimeout;
		if(TxLength ==0)/*add by michael 2013.7.10*/
		{
			DEBUG_CRITICAL("power action not suport");
			return IFD_ERROR_POWER_ACTION;
		}
		DWORD *Action = (DWORD *)TxBuffer;						

		switch (*Action)
		{
			case 0:  	 	/*Power down card*/
				/* Clear ATR buffer */
				CcidSlots[reader_index].nATRLength = 0;
				*CcidSlots[reader_index].pcATRBuffer = '\0';
			...
			case 1:			/*Power on card*/
				...
				*pdwBytesReturned = (nlength < MAX_ATR_SIZE) ? nlength : MAX_ATR_SIZE;
				...
				/* Reset is returned, even if TCK is wrong */
				CcidSlots[reader_index].nATRLength = *pdwBytesReturned;
				
				memcpy(RxBuffer, pcbuffer, *pdwBytesReturned);
				memcpy(CcidSlots[reader_index].pcATRBuffer, pcbuffer, *pdwBytesReturned);
...
/*add escape control modify by michael 2013.6.24*/
