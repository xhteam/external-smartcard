...
{
	int rv;
	int actual_length=0;//modify by michaelZhang 2013.5.29
	char debug_header[] = "<- 121234 ";
	_ccid_descriptor *ccid_descriptor = get_ccid_descriptor(reader_index);
	int duplicate_frame = 0;

        int retrytimes = 0;//modify by michaelZhang  2013.5.29
...
read_again:
	...
	//	*length = 0;
		DEBUG_CRITICAL5("read failed (%d/%d): %d %s",
			...
			usbDevice[reader_index].device_address, rv, strerror(errno));
////////////////////////////////////////////////////////////
// Modify by MichaelZhang Alcor Micro, Corp. 2013.5.29
		if (actual_length > 0)
			goto read_end;
