...
#define IOCTL_SMARTCARD_POWER			SCARD_CTL_CODE(2)
/*modify by michael 2013.6.19*/
#undef DO_PROFILE
#ifdef DO_PROFILE
		...
		*lpBytesReturned = cbRecvLength;
		...
		if (IFD_HVERSION_3_0 == rContext->version)
		{
				...
				cbSendLength, pbRecvBuffer, cbRecvLength, lpBytesReturned);
	                if (IOCTL_SMARTCARD_POWER == dwControlCode)/*add by michael zhang 2013.6.19*/
			{
				DWORD *dwAction = (DWORD *)pbSendBuffer;
				uint32_t readerState;

				readerState = rContext->readerState->readerState;
				/* the protocol is unset after a power */
				rContext->readerState->cardProtocol = SCARD_PROTOCOL_UNDEFINED;
				...
				switch (*dwAction)
				{
					...
					case 1:
						if(rv == SCARD_S_SUCCESS)
						{
							/*clear event of smart card*/
							Log1(PCSC_LOG_DEBUG,"clear event of smart card");
							(void)RFSetReaderEventState(rContext,0);
							memcpy(rContext->readerState->cardAtr, pbRecvBuffer, *lpBytesReturned);
							rContext->readerState->cardAtrLength = *lpBytesReturned;
							readerState = SCARD_PRESENT | SCARD_POWERED | SCARD_NEGOTIABLE;
							rContext->powerState = POWER_STATE_INUSE;
							Log1(PCSC_LOG_DEBUG,"power state in use");
							...
							if (SCARD_PROTOCOL_UNDEFINED == rContext->readerState->cardProtocol)
							{
								...
								DWORD dwPreferredProtocols = SCARD_PROTOCOL_ANY_OLD;
								int ret;

								ucDefault = PHGetDefaultProtocol(rContext->readerState->cardAtr, rContext->readerState->cardAtrLength);
								ucAvailable = PHGetAvailableProtocols(rContext->readerState->cardAtr, rContext->readerState->cardAtrLength);
								...
								if (dwPreferredProtocols & SCARD_PROTOCOL_ANY_OLD)
									...
									ret = PHSetProtocol(rContext, dwPreferredProtocols, ucAvailable, ucDefault);
