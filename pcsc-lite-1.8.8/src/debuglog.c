...
static int syslog_to_android[] = {
	[PCSC_LOG_DEBUG] = ANDROID_LOG_DEBUG,
	[PCSC_LOG_INFO] = ANDROID_LOG_INFO,
	[PCSC_LOG_ERROR] = ANDROID_LOG_ERROR,
	[PCSC_LOG_CRITICAL] = ANDROID_LOG_FATAL,
};
#endif
...
{
	#ifdef ANDROID		
	__android_log_write(syslog_to_android[priority], LOG_TAG, DebugBuffer);
	#else
...
void debug_msg(const char *fmt, ...)
{
	...
	#ifdef ANDROID
	__android_log_write(syslog_to_android[PCSC_LOG_INFO],LOG_TAG,DebugBuffer);
	#else
	if (DEBUGLOG_SYSLOG_DEBUG == LogMsgType)
		syslog(LOG_INFO, "%s", DebugBuffer);
	else
